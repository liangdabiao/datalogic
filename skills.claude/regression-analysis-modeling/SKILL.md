...
description: Perform comprehensive regression analysis and predictive modeling using linear regression, decision trees, and random forests. Use when you need to predict continuous values like customer lifetime value (LTV), housing prices, sales forecasts, or any numerical target variables. Includes automated feature engineering, RFM analysis, model comparison, and visualization with Chinese language support.
...
- **RFM Analysis**: Automatically calculate Recency, Frequency, Monetary values for customer data
...
```
Predict customer lifetime value based on their purchase behavior:
[CSV with user_id, order_date, order_amount, product_category data]
```
...
- **RFM Integration**: Built-in customer value analysis
...
### For Customer LTV Prediction:
- **user_id**: Customer identifier (用户码)
- **date_field**: Transaction date (消费日期)
- **amount_field**: Transaction amount (总价/订单金额)
- **Optional**: Product category, quantity, other customer attributes
...
- **RFM Metrics**: Customer behavior analysis
