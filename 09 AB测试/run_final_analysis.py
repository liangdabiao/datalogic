...
# 写入并执行修正后的脚本
with open('深入分析_final.py', 'w', encoding='utf-8') as f:
    f.write(code)

print("已创建修正后的分析脚本 '深入分析_final.py' 并准备运行...")

# 运行修正后的脚本
import subprocess
result = subprocess.run(['python', '深入分析_final.py'], capture_output=True, text=True, encoding='utf-8')
...
if result.returncode == 0:
    print("\n分析脚本已成功执行完毕。图表和结果已生成。")
else:
    print(f"\n分析脚本执行失败，退出码: {result.returncode}")